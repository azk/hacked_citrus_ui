...
shinyUI(pageWithSidebar(
  ...
  headerPanel(paste("Citrus UI {HACKED} v",citrus.version(),sep=""),windowTitle="Citrus UI"),
  ...
  mainPanel(
    tabsetPanel(
      ...
      tabPanel("Mods",
               ...
               textInput("clusterInputDir","Directory on cluster containing input files","/storage/md_shenorr/"),
               textInput("clusterOutputDir","Direcotry on cluster where output should be saved","/storage/md_shenorr/"),
               tags$br(),tags$br(),
               checkboxInput("bothFeatureTypes","Use both 'abundances' and 'medians' feature types",value=T),
               tags$br(),
               checkboxGroupInput("scriptFlags","Select script parameters",choices=list("DEFINE_PARAMETERS",
                                                                                        "GENERATE_CLUSTERS",
                                                                                        "COMPUTE_FEATURES_AND_DIFFERENCES",
                                                                                        "PLOT_OUTPUT"),
                                  selected=list("DEFINE_PARAMETERS",
                                                "GENERATE_CLUSTERS",
                                                "COMPUTE_FEATURES_AND_DIFFERENCES",
                                                "PLOT_OUTPUT"))               
      ),
